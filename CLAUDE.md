...
| `stays.html` | Accommodations overview — one card per property (sleeping pods, Rupa Rupa, Aranwa, Hotel Indigo) with dates, address, room, check-in/out, cost, and the open flags. Reached from a `.stays-link` panel in `index.html`, between the elevation chart and "Day by day". |
| `stays/` | Property photos. A jpg named `<key>.jpg` (`pods`, `rupa-rupa`, `aranwa`, `indigo`) is picked up automatically; when it is missing the card's `onerror` swaps in a "see photos" placeholder linking to an image search. See `stays/README.md`. |
...
- **At a glance** (overview cards): three cards per row; 11.5px item text. The owner calls this section the **AAG**.
  - Status dots: `status-booked` green = booked, `status-needed` red = still to book, `status-partial` **orange `#E08A3C` = booked but needs fixing** (the Machu Picchu buses, dated Sun 9/27 rather than Sat 9/26 — Rohan expects them to be accepted anyway), `status-na` grey = walk-up / nothing to book. Keep the legend row in step with these.
...
- **Each day heading carries a "Map" pill** (`.day-head-row` > `.day-heading` + `.day-map`) linking to `maps.html?day=N`. The pill sits just right of the date (`margin-left:16px; margin-right:auto`). It has been both edge-pinned and centred; both looked wrong on a wide laptop window, so leave it beside the heading. `N` is the same zero-based index as the `day-jump` links, so **renumber both together** if a day is ever added or reordered.
...
- **`maps.html` carries the same photo background as the site** — same `body::before` recipe, same `--lift`/`--dim-text`, same glyph halo. Content sits on `rgba(0,0,0,0.44)` panels so it stays legible, and **dark-on-gold elements opt out of the halo** (`.gbtn`, `ol.stops li::before`, the active day chip, `.pin`) or they read as a smudge.
- **Inside `maps.html`, anything dark-on-light must opt out of the glyph halo** (`text-shadow:none`) — the halo is for pale text on the photo, and over dark text it reads as a smudge. That covers the gold Google button, the numbered list markers, the active day pill, the pins, and all of Leaflet's own chrome: popups, tooltips, the close button, the zoom bar and the attribution. Leaflet's white popups are additionally restyled to the site's dark panel (`#1E211A`, cream text, gold titles).
- **Merged pins** (several stops at one place) are labelled with **slashes — "1/6/9"** — and `white-space:nowrap`. Commas wrapped the label onto two lines; a middle dot read as a hyphen at pin size. The pin itself is `position:absolute` + `translate(-50%,-50%)` **inside** Leaflet's marker box: that box is sized to `iconSize` (24×24) and a plain child cannot grow past it, which was the real cause of the wrap. Absolute centring lets a wide label shrink-wrap and still sit on the anchor.
- **`stays.html` never embeds a photo it cannot verify.** Every image host was blocked from the build sandbox (Unsplash, IHG, Expedia, Booking, Wikimedia), so no property photo could be downloaded. Rather than ship a stock image pretending to be the room, each card's photo slot links to a live image search and names the filename to drop in. **If photos are added later, prefer ones the owner took or the property publishes for guest use — the repo is public.**
...
- Day sublines in the **day-by-day** section (`.day-heading-sub`) use the same bright gold `#E6BE63` at weight 600 as the at-a-glance route lines, so the two sections read as one system.
- **Detailed itinerary** is headed **"Day by day"** (mirrors "At a glance", same `.overview-heading` / `.overview-sub` classes, **23px** since Sep 21 — one class drives both titles, so they cannot drift apart; note this sits only 1px above the 22px `.day-heading`). Its subline doubles as the swipe hint.
...
- **Leg times appear in both places.** `maps.html` has `.leg` in its stop list and as a tooltip on each map segment; `index.html` has `.stop-leg`, the first child of each `.stop`, so it sits between the previous stop's Details and this stop's time. **The two lists are indexed differently** — `LEGS` in `maps.html` is keyed by position in the *mapped* list (flights excluded), while the day panels have their own stop counts, so a leg added or reworded in one does not propagate to the other. Update both.
  - `.stop-leg` is **blue `#A9D4FF`**, the one place the page leaves the gold-and-cream system, so a travel time separates from the gold route lines by hue rather than by weight. Measured against the photo's brightest 5%: **3.61:1, better than the gold it replaced (3.17)**. A saturated blue (`#4A9EFF`) was measured first at **2.03 and rejected** — do not darken this. **`maps.html` uses the same blue** for `.leg` and for the map-segment tooltips, so a travel time reads identically on both pages; on that page's 0.44 panel it measures **7.33:1** against the gold's 6.44. Gold stays for the pins, the numbered markers and the Google button.
  - The timeline dot hangs off `.stop-time::before`, **not** `.stop::before` — otherwise a leg line above the time drags the dot up with it.
  - A leg is emitted only where there is real movement, and never on a day's first stop (there is no previous stop on the page to travel from).
  - **The leg is centred in the gap, not hugging the time below it.** `.stop` normally has `padding-bottom:26px`; `.stop:has(+ .stop .stop-leg)` drops that to 19px and the leg carries a 19px bottom margin, so the gap splits evenly (19/19) around it while the total stays the 38px it always was. Stops with no leg after them keep the full 26px. Without `:has` support the page falls back to the old 26/12 — not centred, but no regression. Measured, not eyeballed.
- **Type scale in the day-by-day stops** (raised Sep 21 on the owner's request): title 19.5px / `.stop-time` **16px** / `.stop p` body 15px / `.stop-leg` and `.stop-details summary` **14px**. The timestamp is deliberately **larger than the body copy** — it is the thing you scan for, not something read in sequence.

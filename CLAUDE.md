...
- **Each day heading carries a "Map" pill** (`.day-head-row` > `.day-heading` + `.day-map`) linking to `maps.html?day=N`. The pill uses `margin:0 auto` inside the flex row so it sits **midway between the end of the date and the right edge**, not jammed against the edge — do not swap this for `justify-content:space-between`. `N` is the same zero-based index as the `day-jump` links, so **renumber both together** if a day is ever added or reordered.

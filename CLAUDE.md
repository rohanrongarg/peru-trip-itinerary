...
- **Background:** one **sharp** fixed photo, never blurred, barely dimmed (`--dim-text` 0.12). The cards and detail panels get only `--dim-card` 0.20 — enough to separate them, deliberately not enough to read as boxes over the photo. Legibility comes from the glyph halo in `.wrap { text-shadow }`. Nothing is keyed to a fixed pixel width, so phones and desktops match.

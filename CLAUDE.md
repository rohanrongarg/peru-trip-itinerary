...
- **Background:** one **sharp** fixed photo, never blurred. Contrast is applied where the text is, not across the whole image: `--dim-text` (0.14) is a light base overlay, `--hero-scrim` fades a darker wash behind the hero and is gone by 74% height, and `--dim-card` (0.50) deepens the cards and detail panels that hold the small text. Glyphs carry a halo via `.wrap { text-shadow }`. Nothing is keyed to a fixed pixel width, so phones and desktops get the same treatment. All three values are **measured** worst-case minimums at a 15%-relaxed WCAG target across 390/768/1024/1440. **If you change the photo or any of these values, re-measure** with `blend-lab.html`. Don't guess.

...
- **Background:** one **sharp** fixed photo, never blurred, barely dimmed (`--dim-text` 0.12). The cards and detail panels get only `--dim-card` 0.20 — enough to separate them, deliberately not enough to read as boxes over the photo. Legibility comes from the glyph halo in `.wrap { text-shadow }` — note `<footer>` sits **outside** `.wrap` and carries its own copy, and `svg.chart text` uses `paint-order:stroke fill` instead, which is far stronger at label sizes. Secondary text is `--text-muted` `#D2CAB0` (lifted from `#B5AC92`, which disappeared into the photo). Nothing is keyed to a fixed pixel width, so phones and desktops match.
